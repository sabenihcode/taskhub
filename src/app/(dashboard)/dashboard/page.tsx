    ...
    async function load() {
      try {
        const [statsRes, requestsRes] = await Promise.all([
          ...
          fetch("/api/requests?limit=5", { credentials: "same-origin" }),
        ]);
        ...
        if (requestsRes.ok) {
          ...
          if (Array.isArray(requestsJson)) {
            items = requestsJson;
          } else if (requestsJson?.items && Array.isArray(requestsJson.items)) {
            items = requestsJson.items;
          } else if (requestsJson?.data && Array.isArray(requestsJson.data)) {
            items = requestsJson.data;
          }
    ...
    <div className="space-y-8">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-2">
              ...
              <p className="text-teal-50/90 text-base">
                Anda memiliki <strong className="font-semibold text-white">{activeRequests} request aktif</strong> yang perlu ditangani
              </p>
            </div>

            <div className="hidden sm:block">
              <div className="rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20 p-4 text-center min-w-[120px]">
                <p className="text-xs font-medium text-teal-100 uppercase tracking-wider">Total</p>
          ...
          <div className="relative flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-amber-700 mt-0.5">
                Lebih dari 3 hari tidak ada aktivitas. Segera tinjau dan berikan update.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Status request berdasarkan tahap proses</p>
        </div>
      </div>
      ...
      <div>
        <div className="mb-4 flex items-end justify-between">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Request Terbaru</h2>
            <p className="text-sm text-slate-500 mt-0.5">5 request terakhir yang dibuat</p>
          </div>
          <Link
            href="/requests"
